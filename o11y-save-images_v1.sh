...
pull_image() {
  ...
  if docker pull "${image}" > /dev/null 2>&1; then
    echo -e "${GREEN}Image pull success${NO_COLOR}: ${image}" >&2
    echo "${image}"
  else
    echo -e "${RED}Image pull failed${NO_COLOR}: ${image}" >&2
  fi
}
...
pulled=$(cat "${image_list_file}" | xargs -P 4 -I {} bash -c 'pull_image "{}"' | tr '\n' ' ')

echo -e "Creating ${image_archive} with $(echo "${pulled}" | wc -w | tr -d '[:space:]') images"
# shellcheck disable=SC2086
docker save ${pulled} | gzip -1 > "${image_archive}"
